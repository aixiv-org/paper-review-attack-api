...
def main():
    parser = argparse.ArgumentParser(description='运行提示词注入检测')
    ...
    try:
        ...
        if malicious_results:
            ...
            for i, result in enumerate(malicious_results[:10], 1):  # 只显示前10个
                ...
                for detection in result.get('detections', []):
                    detection_types.add(detection['type'])
                
                logger.info(f"{i:2d}. {file_name}")
                logger.info(f"    风险分数: {risk_score:.3f}")
                logger.info(f"    检测类型: {', '.join(detection_types)}")
                logger.info("")
            ...
            logger.info(f"恶意文件详情已保存: {malicious_file}")
        ...
        if detection_type_counts:
            logger.info("\n📊 检测类型统计:")
            for det_type, count in sorted(detection_type_counts.items(), 
